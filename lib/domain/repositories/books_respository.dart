import 'package:cloud_firestore/cloud_firestore.dart' show CollectionReference, DocumentReference, DocumentSnapshot, FieldPath, FirebaseFirestore, Query, QueryDocumentSnapshot, QuerySnapshot, SetOptions;
...
class BooksRepository {
  ...
  Future<Book?> fetchBookByISBN(final String isbn) async {
    final DocumentSnapshot<Map<String, dynamic>> doc = await _booksCollection.doc(isbn).get();
  ...
  Future<List<Copy>> fetchCopiesByBook(final String book) async {
    final QuerySnapshot<Map<String, dynamic>> querySnapshot = await _copiesCollection
        .where(Copy.bookString, isEqualTo: book)
        .get();
  ...
  Future<String> addCopy(final String book) async {
    final DocumentReference<Map<String, dynamic>> docRef = await _copiesCollection.add(<String, dynamic>{
  ...
  Future<(List<Book> books, DocumentSnapshot<Map<String, dynamic>>? lastDoc)> fetchBooksPage({
    final int limit = 10,
    ...
    final List<Book> books = querySnapshot.docs.map(
      (final QueryDocumentSnapshot<Map<String, dynamic>> doc)
        => Book.fromMap(doc.data(), doc.id),
    ).toList();

    final QueryDocumentSnapshot<Map<String, dynamic>> lastDoc = querySnapshot.docs.last;
